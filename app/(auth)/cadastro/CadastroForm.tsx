  ...
  if (ok) return (
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="text-center anim-fade-up">
        ...
        <Btn
          onClick={() => router.push(`/login?role=${isRest ? 'restaurante' : 'colaborador'}`)}
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-8">
      <div className="anim-fade-up w-full max-w-[380px]" style={{
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <Input
            label={isRest ? 'Nome do restaurante' : 'Seu nome'}
            value={nome}
            onChange={e => setNome(e.target.value)}
            placeholder={isRest ? 'Restaurante Bom Sabor' : 'João Silva'}
          />
          <Input
            label="E-mail"
            type="email"
