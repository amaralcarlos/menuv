export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await sb
    .from('empresas')
    .select('id, nome, horario_limite, extensao_ate, preco_por_refeicao, formato, ativa, status_plano, restaurante_id, dia_ciclo')
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const diaCiclo = parseInt(body.diaCiclo ?? '1') || 1
  ...
  const { error } = await sb.from('empresas')
    .update({ nome, horario_limite: hl, preco_por_refeicao: preco, ativa, formato, dia_ciclo: diaCiclo }).eq('id', id)
