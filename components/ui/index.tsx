...
// ── PasswordInput ────────────────────────────────────────────
export function PasswordInput({ label, error, className = '', ...props }: Omit<InputProps, 'type'>) {
    ...
    <div className="flex flex-col gap-1.5">
      ...
      <div className="relative">
        <input
          ...
          className={`w-full bg-[#080c14] border border-[#253d5e] rounded-[11px] px-3 py-2.5 pr-10 font-[var(--mono)] text-[#ddeaf8] outline-none transition-all duration-150 focus:border-[rgba(0,232,122,.5)] focus:shadow-[0_0_0_3px_rgba(0,232,122,.06)] placeholder:text-[#3d5875] ${className}`}
        />
        <button
          type="button"
          onClick={() => setShow(v => !v)}
          className="absolute right-3 top-1/2 -translate-y-1/2 text-[#3d5875] hover:text-[#7a96b8] transition-colors cursor-pointer bg-transparent border-none p-0"
          tabIndex={-1}
